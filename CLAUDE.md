...
- A project skill lives at `.claude/skills/markdown-clipping/`. It gives
  profile-separated CommonMark / GFM / Obsidian Markdown guidance for
  design/implementation/review of rendering, capture fixtures, and Obsidian
  export, plus an offline example verifier (`pnpm run skill:verify`, also run
  in CI). It is development-time guidance only — never a runtime dependency
  ...
  deterministic authority path, `decisions/0002`).
