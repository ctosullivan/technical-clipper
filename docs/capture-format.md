...
As of Phase 5, `packages/pipeline` populates a `DocumentIR` from a rendered
article DOM (`decisions/0022`–`0025`): deterministic article-root selection,
structural-noise removal recorded as `removedRegions`, DOM → block/inline
nodes, code/terminal detection (`code/pre-code`, Prism, Highlight.js,
`terminal/session`, virtualized-editor guard), protected-code restoration at
sentinels, and the `documentContentIdentity` plus per-code-block hashes. The
Markdown and bundle files are still Phase 7.
