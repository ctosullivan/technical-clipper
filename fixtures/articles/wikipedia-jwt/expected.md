...
source_url: https://en.wikipedia.org/wiki/JSON_Web_Token
canonical_url: https://en.wikipedia.org/wiki/JSON_Web_Token
...
| `exp` | Expiration Time | Identifies the expiration time on and after which the JWT **must not** be accepted for processing. The value must be a NumericDate:[9] either an integer or decimal, representing seconds past [1970-01-01 00:00:00Z](https://en.wikipedia.org/wiki/Unix_time "Unix time"). |
...
| `typ` | Token type | If present, it must be set to a registered [IANA Media Type](https://www.iana.org/assignments/media-types/media-types.xhtml). |
